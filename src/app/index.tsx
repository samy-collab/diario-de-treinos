...
export default function WorkoutsScreen() {
  ...
  useEffect(() => {
    ...
    return subscribeToWorkouts(
      ...
      () => {
        setError(
          'Não foi possível carregar os treinos. Confira sua internet e as regras do Firestore.',
        );
        setIsLoading(false);
      },
    );
  ...
  const visibleWorkouts = useMemo(
    () => workouts.filter((item) => activity === ALL || item.activity === activity),
    [activity, workouts],
  );
  // Regra de negocio: estatisticas mensais calculadas com dados do Firestore.
  const monthWorkouts = useMemo(
    () => workouts.filter((item) => item.date.startsWith(currentMonth())),
    [workouts],
  );
  ...
  async function handleSaveProfile() {
    ...
    if (!Number.isInteger(age) || age < 1 || age > 120)
      return setProfileError('Informe uma idade válida entre 1 e 120 anos.');
    if (!user) return;
    try {
      ...
      <View style={styles.container}>
        <View style={styles.headerRow}>
          <View>
            <Text style={styles.greeting}>
              {profile?.name ? `Olá, ${profile.name}!` : 'Olá, vamos treinar?'}
            </Text>
        ...
        {!profile ? (
          <View style={styles.profileCard}>
            <Text style={styles.profileTitle}>Complete seu perfil</Text>
            <Text style={styles.profileHelp}>
              Informe seus dados para personalizar o aplicativo.
            </Text>
            <View style={styles.profileFields}>
              ...
              <TextInput
                ...
                placeholder="Idade"
                style={[styles.profileInput, styles.ageInput]}
                value={profileAge}
              />
            </View>
        ...
        <View style={styles.summary}>
          <Text style={styles.summaryLabel}>RESUMO DESTE MÊS</Text>
          <View style={styles.summaryRow}>
            <View>
              <Text style={styles.summaryValue}>{monthWorkouts.length}</Text>
              <Text style={styles.summaryUnit}>treinos</Text>
            </View>
            <View style={styles.divider} />
            <View>
              <Text style={styles.summaryValue}>{monthMinutes}</Text>
              <Text style={styles.summaryUnit}>minutos</Text>
            </View>
          </View>
          <Text style={styles.summaryHelp}>Resumo calculado com os treinos do Firestore</Text>
        </View>
        <Text style={styles.filterLabel}>Filtrar por modalidade</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filters}>
          {[ALL, ...WORKOUT_TYPES].map((item) => (
            <Pressable
              key={item}
        ...
        {isLoading ? (
          <View style={styles.center}>
            ...
            <Text style={styles.muted}>Carregando treinos...</Text>
          </View>
        ) : (
          <FlatList
            contentContainerStyle={visibleWorkouts.length ? styles.list : styles.emptyList}
            data={visibleWorkouts}
            keyExtractor={(item) => item.id}
            ListEmptyComponent={
              <View style={styles.empty}>
                <Text style={styles.emptyTitle}>
                  {workouts.length ? 'Nenhum treino neste filtro' : 'Nenhum treino cadastrado'}
                </Text>
                <Text style={styles.muted}>
                  {workouts.length
                    ? 'Escolha outra modalidade.'
                    : 'Registre seu primeiro treino para acompanhar sua evolução.'}
                </Text>
              </View>
            }
            renderItem={({ item }) => (
        ...
        <Pressable onPress={() => router.push('/workout-form')} style={styles.addButton}>
          <Text style={styles.addButtonText}>+ Registrar treino</Text>
        </Pressable>
        <ConfirmationModal
          confirmLabel="Sair"
...
const styles = StyleSheet.create({
  ...
  greeting: { color: '#14532d', fontSize: 21, fontWeight: '800' },
  email: { color: '#64748b', fontSize: 13, marginTop: 2 },
  logout: { color: '#dc2626', fontWeight: '700', padding: 8 },
  profileCard: {
  ...
  profileTitle: { color: '#1e3a8a', fontSize: 16, fontWeight: '800' },
  profileHelp: { color: '#64748b', fontSize: 12, marginBottom: 10, marginTop: 3 },
  profileFields: { flexDirection: 'row', gap: 8 },
  profileInput: {
    backgroundColor: '#fff',
    borderColor: '#cbd5e1',
    borderRadius: 9,
  ...
  ageInput: { flex: 0, width: 82 },
  profileError: { color: '#b91c1c', fontSize: 12, marginTop: 8 },
  profileButton: {
    alignItems: 'center',
  ...
  summary: { backgroundColor: '#15803d', borderRadius: 20, marginBottom: 16, padding: 20 },
  summaryLabel: { color: '#bbf7d0', fontSize: 12, fontWeight: '800', letterSpacing: 1 },
  summaryRow: { flexDirection: 'row', gap: 24, marginVertical: 10 },
  summaryValue: { color: '#fff', fontSize: 30, fontWeight: '900' },
  summaryUnit: { color: '#dcfce7' },
  divider: { backgroundColor: '#4ade80', width: 1 },
  summaryHelp: { color: '#dcfce7', fontSize: 12 },
  filterLabel: { color: '#334155', fontSize: 13, fontWeight: '700' },
  filters: { flexGrow: 0, marginBottom: 16, marginTop: 9 },
  chip: {
